...
for x in range(1) :  # crée une liste des liens des jeux par nombre de page
    url = f"https://www.gog.com/fr/games?page={x}"
...
#for link in range(5) :
    ...
    for howlong in soup.find_all('span'):
        ...
        # Récupére la date de sortie du jeux : NOT NULL
    x = 0
...
for game,values in games_dict.items():
    ...
    #liste des differentes langues des jeux 
    ...
    #liste des differents genre des jeux 
...
for game,values in games_dict.items():
    ...
    for language in games_languages:
        #recupere l'id des langues
        sql = "SELECT id FROM languages WHERE language = (%s)"
        val = (language)
        mycursor.execute(sql, (val,))
        language_game = mycursor.fetchone()[0]

        #insert dans la table games_languages l'id des jeux et des langues 
    ...
    for genre in games_genres:
        ...
        #insert dans la table games_genres l'id des jeux et des genres 
